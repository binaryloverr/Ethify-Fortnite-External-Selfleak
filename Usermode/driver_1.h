	...
	const bool Init(const BOOL PhysicalMode) {
		...
		if (this->hDriver != INVALID_HANDLE_VALUE) {
			...
			if (this->SharedBuffer = VirtualAlloc(0, sizeof(REQUEST_DATA), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)) {
				SPOOF_FUNC;
				UNICODE_STRING RegPath = RTL_CONSTANT_STRING(L"\\Registry\\Machine\\SOFTWARE\\Discord");
...
private:
	PVOID SharedBuffer;
	HANDLE hDriver;
	ULONG64 MAGGICCODE = 0x06bbc0f31c8e5c20;
...
template<typename T>
inline bool read_array(uintptr_t address, T* array, size_t len) {
	SPOOF_FUNC;
	//return Drive.ReadPtr(pid, address, array, sizeof(T) * len);
	return driver->ReadProcessMemory(address, &array, sizeof(T) * len);
}

//template<typename T>
//inline bool read_array(uintptr_t address, T* array, size_t len) {
//	//return Drive.ReadPtr(pid, address, array, sizeof(T) * len);
//	driver->ReadProcessMemory(address, &array, sizeof(T) * len);
//
//	return 1;
//}
